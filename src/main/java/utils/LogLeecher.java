...
/**
 * A Leecher can attach to a and wait until a specific text is printed in the log.
 */
public class LogLeecher {
     ...
     */
    public void waitForText(long timeout) throws Exception {
            ...
            while (!textFound && !forcedExit) {
                try {
                    ...
                    if (System.currentTimeMillis() - startTime > timeout) {
                        throw new Exception("Timeout expired waiting for matching log: " + text);
                    }
                } catch (InterruptedException e) {
                    throw new Exception("Error waiting for text", e);
                }
            }

            if (!textFound) {
                throw new Exception("Matching log not found prior to server shutdown for: " + text);
            }
        }
    }
